...
const Navbar = () => {
  ...
  const Links = [
    { name: 'HOME', link: '/' },
    { name: 'Stage-1', link: '/round1' },
    { name: 'Stage-2', link: '/round2' },
    { name: 'Dash', link: '/Home' },
    { name: 'R', link: '/Re' }
  ];
      ...
      >
        ...
        >
          {Links.map((link, index) => (
            ...
            >
              <a href={link.link} className="text-white hover:text-purple-700 duration-500">{link.name}</a>
            </motion.li>
          ))}
          <motion.button
            initial="hidden"
            animate="visible"
            variants={linkVariants}
            className="btn bg-purple-600 text-white md:ml-8 font-semibold px-3 py-1 rounded duration-500 md:static cursor-pointer"
            onMouseEnter={textEnter}
            onMouseLeave={textLeave}
          >
